...
## 2026-03-15 (#7): 射手歸因 140 輪迭代後的教訓 — 簡單方法勝過過度工程

### 問題
經過 140 輪迭代改進射手歸因（Trace+Extrapolation+Proximity+Formation+Direction+Sigmoid 校準等 ~900 行），系統變得極其複雜但準確度仍無法驗證。每輪迭代加入的微調（如 +-0.03 信心度調整、Alliance tie-breaker、Rate limiting 等）相互干擾，難以理解整體行為。

### 根因
過度工程（over-engineering）：在沒有 ground truth 數據驗證的情況下，盲目增加啟發式規則。每條規則都「看起來合理」，但組合後產生不可預測的交互作用。

### 解決方案
大幅簡化為兩條清晰規則：
1. **HP 進球**：球軌跡穿越 HP 線段 → HP 進球（conf=0.95）
2. **非 HP 進球**：沿軌跡回溯到 `goal_frame - 80` 的球位置，找該幀最近的機器人

新方法 `_simple_proximity_attribution()` 約 20 行，取代原本 ~900 行。

### 選擇理由
- **可理解性**：兩條規則，任何人都能理解歸因邏輯
- **可調試**：歸因錯誤時，只需檢查球位置和機器人位置
- **先簡單再複雜**：如果簡單方法準確度不夠，再有針對性地加入改進，而非從一開始就堆疊 140 條規則
- **舊方法保留**：`_find_shooter_by_trajectory` 等舊方法仍被 `detect_shots` 使用，暫不刪除

---

## 2026-03-15 (#6): MOT Greedy 匹配 → Hungarian 算法 — 解決靠近時 ID 交換

### 問題
兩台機器人靠近時（如在 hub 附近交錯），追蹤 ID 會交換（Robot A 的 ID 跳到 Robot B 上，反之亦然）。

### 原因
Round 1 ACTIVE 匹配使用 **greedy 排序**（按距離排序後逐一匹配），這是局部最優策略。當兩台機器人與兩個偵測距離接近時，greedy 可能先把偵測 D1 匹配給 Robot A（因為 A→D1 距離最短），但全域最優應該是 A→D2、B→D1（總距離更小）。Greedy 的匹配順序依賴排序，容易在距離接近時產生不穩定的結果。

### 解決方案
Round 1 改用 **Hungarian 算法**（`scipy.optimize.linear_sum_assignment`）：
1. 建立 cost matrix `C[i,j]` = 偵測 i 到 ACTIVE 標籤 j 的加權距離（含 IoU bbox 加權）
2. 超過 `MOT_ACTIVE_MAX_DIST` 的設為 `1e9`（禁止匹配）
3. `linear_sum_assignment(C)` 求全域最優匹配（O(n^3)，12 台機器人完全可接受）
4. 過濾掉 cost=1e9 的匹配

### 選擇理由
- **Hungarian vs Greedy**: 機器人靠近時 greedy 不穩定，Hungarian 保證全域最優。計算量 O(n^3) 對 n<=12 的 FRC 場景完全可忽略。
- **只改 Round 1**: Round 2 LOST 復活仍用原邏輯（LOST 數量少且有 Re-ID 門檻保護，不需要全域最優）
- **SciPy 依賴**: 專案已使用 SciPy（球追蹤的 CentroidTracker 也用 Hungarian），無新增依賴

---

## 2026-03-14 (#5): 射手歸因根本性重寫 — 軌跡起源外推取代固定回溯距離

### 問題
射手歸因精確度接近 0。明明是 Robot A 射的球，卻歸因到離球較近的 Robot B。

### 根因
舊的 Proximity 歸因邏輯有**根本性設計缺陷**：
1. **看球的位置而非球從哪來** — 取 `goal_frame - 80` 的球位置，找最近的機器人。但球在飛行中可能已離開射手很遠，途中經過的其他機器人反而更近。
2. **完全忽略飛行方向** — 沒有利用球的速度向量來推斷發射位置。
3. **未外推發射位置** — 球首次偵測時可能已在空中飛行，軌跡起點 ≠ 發射位置。

**具體場景**：Robot A 在 (200,400) 射球，球首次偵測在 (500,350)，Robot B 在 (450,380)。舊方法：Robot B 距球 58px < Robot A 距球 304px → 歸因 Robot B ❌

### 解決方案
新增 `_find_shooter_by_trajectory()` 方法，用**軌跡起源分析**取代固定回溯距離：

**兩段式策略：**
1. **方法 A（Pre-shot Origin）**：從後往前掃描球軌跡速度，找到低速段（`speed < SHOT_MIN_VELOCITY * 0.5`）= 球在射手附近的最後時刻。直接用該位置找最近機器人。
2. **方法 B（Velocity Extrapolation）**：若球首次偵測就在高速飛行中，用前 5 段平均速度向後外推 `SCORE_EXTRAPOLATE_FRAMES=20` 幀估計發射位置。

**方向感知評分**（僅外推模式使用）：
- 計算「機器人→球首次偵測位置」向量與「球飛行方向」的 cosine similarity
- `score = dist * (1 - SCORE_DIRECTION_WEIGHT * cos_sim)`
- cos_sim ≈ 1（球正飛離此機器人）→ score 降低 30%
- cos_sim ≈ -1（球正飛向此機器人）→ score 提高 30%

**同場景修復後**：外推位置 (100,410)，Robot A 距 101px，Robot B 距 351px → 歸因 Robot A ✅

### 新增常數
- `SCORE_EXTRAPOLATE_FRAMES = 20`（外推幀數，30fps 下 0.67 秒）
- `SCORE_DIRECTION_WEIGHT = 0.3`（方向修正權重）
- `SCORE_MIN_VELOCITY_FOR_DIR = 5`（方向評分最低球速門檻）

### 測試結果
| 場景 | 舊方法 | 新方法 |
|------|--------|--------|
| 飛行中球（Robot A 遠、B 近）| Robot B ❌ | Robot A ✅ (extrapolation) |
| 低速起源（球先在 A 附近）| Robot B ❌ | Robot A ✅ (origin) |
| 短軌跡（2 點）| 隨機 | 合理外推 |

---

## 2026-03-14 (#4): HP 歸因三層判定 — 解決球軌跡飛過 HP 線段的漏判

### 問題
1. HP 丟的球卻被歸因到機器人身上（假陽性）
2. 藍方 HP 進球數顯示 0（假陰性）

### 原因
球在 HP 手中或剛拋出時未被偵測（被 HP 的身體遮擋、或球速過快導致模糊），導致球軌跡的起點已經飛過 HP 線段。原本的 `_check_hp_attribution()` 只檢查線段交叉，球軌跡不穿過 HP 線段就無法歸因為 HP 進球。同時，`SCORE_HP_PROXIMITY=50` 過嚴，距離容忍太小導致近距離飛過的球也被漏判。

### 解決方案
`_check_hp_attribution()` 重構為**三層判定**（任一層通過即歸因為 HP）：
1. **Layer 1 — 線段交叉**: 球軌跡（連續兩點）與 HP 線段交叉（原邏輯，最精確）
2. **Layer 2 — 距離容忍**: 球軌跡任一點距 HP 線段 ≤ `SCORE_HP_PROXIMITY=150px`（放寬容忍度）
3. **Layer 3 — 起源判定**: 球軌跡最早 3 點距 HP 線段 ≤ `SCORE_HP_ORIGIN_DIST=400px`（最寬鬆，處理起點已飛過的情況）

### 選擇理由
- **三層 vs 單層**: 單一判定方式無法覆蓋所有場景。線段交叉最精確但球軌跡可能跳過；距離容忍處理近距離飛過；起源判定處理遠距離起點。三層遞進降低門檻。
- **起源判定只看前 3 點**: 限制在軌跡最早期，避免球飛行中經過 HP 線段附近被誤判。
- **SCORE_HP_PROXIMITY 50→150**: 50px 在 720p 畫面上太嚴，HP 線段本身可能有標定誤差，150px 約為 bumper 寬度的 2-3 倍，合理容忍。

---

## 2026-03-14 (#3): 遮擋區域追蹤框跳躍 — ACTIVE 匹配距離上限 + IoU 加權

### 問題
機器人進入遮擋區域（hub 附近）時，追蹤框跳到場地另一端的其他機器人上。

### 原因
Round 1 ACTIVE 匹配使用貪心策略，當機器人被遮擋（連續幾幀未偵測到），凍結位置與最近偵測之間的距離增大。此時場地另一端出現的偵測可能成為「最近」匹配（因為沒有距離上限），導致追蹤框跳躍。

### 解決方案
1. **`MOT_ACTIVE_MAX_DIST=200px` 距離上限**: Round 1 ACTIVE 匹配超過此距離直接拒絕，軌跡轉為 LOST 而非搶遠處偵測
2. **IoU bbox 加權**: 匹配距離加入 bbox 重疊度獎勵，位置接近且 bbox 重疊的匹配優先
3. **遮擋區域附近 grace 降為 1 幀**: 遮擋區域邊緣的 ACTIVE→LOST 轉換更快，避免遮擋期間仍以 ACTIVE 身份搶偵測

### 選擇理由
- **距離上限 vs 無限制**: 200px 約為機器人寬度的 2-3 倍，正常追蹤不會超過此距離（30fps 下機器人單幀移動 <50px），超過一定是匹配錯誤
- **IoU 加權 vs 純距離**: 兩機器人接近時中心距離可能接近，但 bbox 重疊度可以區分「同一台」和「不同台但靠近」

---

## 2026-03-14 (#2): MOT 追蹤框搶 ID — 全幀+tiled 重複偵測 + 5 層去重防線

### 問題
MOT 追蹤框搶 ID：兩個追蹤框同時追蹤同一台機器人，互相搶偵測結果導致 ID 交替閃爍。

### 原因
根因是 `detect_raw()` 中全幀偵測（640x640 縮放）和 tiled 偵測（1280px tile）產生**不同尺寸**的 bbox：
1. 全幀偵測的 bbox 較小（因為 640x640 壓縮後解析度低）
2. tiled 偵測的 bbox 較大（因為 1280px tile 保留更多細節）
3. 兩個 bbox 中心接近但尺寸差異大 → NMS IoU<0.5 → 無法合併
4. 加上 tiled 是前一幀提交的非同步結果，位置有偏移
5. 重複偵測在 Round 1 被分配給不同標籤 → 兩框追同一台
6. 之前嘗試的速度外推凍結沒效果，因為 `missed_frames` 在第一幀丟失時仍為 0

### 解決方案
**5 層去重防線**（多層防禦，確保重複偵測不會造成問題）：
1. **Round 1 凍結位置** — 所有 ACTIVE 軌跡匹配用凍結位置（last_cx, last_cy），不做速度外推，避免飄移搶別人偵測
2. **偵測端中心距離去重** — `detect_raw()` 在 NMS 之後加中心距離去重（100px），同中心不同尺寸的重複偵測只保留一個
3. **幽靈偵測守衛** — `_match_direct()` 中未匹配偵測要建新標籤前，檢查 100px 內是否已有匹配偵測，有就跳過
4. **Round 2 鄰近守衛** — LOST 復活時檢查偵測離已匹配偵測 <80px 則跳過
5. **每幀 bbox IoU dedup** — 兩軌跡 bbox IoU>0.5 移除偵測幀少的（最後防線）

### 選擇理由
- **多層防線 vs 單一修復**: 重複偵測的來源多樣（全幀/tiled 尺寸差、非同步位置偏移），單一修復無法覆蓋所有情況
- **中心距離去重 vs 只靠 NMS**: NMS 依賴 IoU，而不同尺寸 bbox 的 IoU 天然偏低，中心距離去重更直接有效
- **凍結位置 vs 速度外推**: 速度外推在機器人快速移動時會把預測位置推到其他機器人附近，凍結位置更安全（代價是快速移動時可能丟幀，但 grace period 會補救）

### 關鍵發現
- 全幀和 tiled 偵測的 bbox 尺寸差異是 NMS 無法處理的盲區 — NMS 假設同一物體的 bbox 尺寸相近，但多尺度偵測打破了這個假設
- `missed_frames` 在第一幀丟失時為 0，所以之前基於 `missed_frames > 0` 的凍結邏輯完全無效
- 射手歸因同步簡化為 2 層（HP > Proximity），移除 Ball Ownership 層 — 因為球所有權在追蹤不穩定時反而引入噪音
